...
import (
	"github.com/capsocrates/advent-of-code/day02"
...
func BenchmarkParseDay02(b *testing.B) {
	for i := 0; i <= b.N; i++ {
...
func BenchmarkValidateDay02Solo(b *testing.B) {
	for i := 0; i <= b.N; i++ {
...
func TestDay02(t *testing.T) {
	t.Run("ExampleInput", func(t *testing.T){
		...
		require.Nil(t, err)
		assert.Equal(t, 2, validPwds)
	})
	t.Run("ProblemInput", func(t *testing.T) {
		validPwds, err := day02.ValidatePasswords(problemInputs)

		require.Nil(t, err)
		assert.Equal(t, 383, validPwds)
	})
}
